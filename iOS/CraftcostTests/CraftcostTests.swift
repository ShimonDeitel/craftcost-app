import XCTest
@testable import Craftcost

@MainActor
final class CraftcostTests: XCTestCase {
    ...
    func testCannotAddMoreAtLimitWhenFree() {
        let store = Store()
        store.isPro = false
        store.items = (0..<Store.freeLimit).map { _ in Purchase() }
    ...
    func testProUsersAlwaysCanAddMore() {
        let store = Store()
        store.isPro = true
        store.items = (0..<(Store.freeLimit + 10)).map { _ in Purchase() }
        XCTAssertTrue(store.canAddMore)
    }
